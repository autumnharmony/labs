...
namespace GBusManager
{
    ...
    public class PriorityQueue<TPriority, TValue> : ICollection<KeyValuePair<TPriority, TValue>>
    {
        ...
        {
            Console.WriteLine("В очередь добавлено {1} c  приоритетом {0}", priority, value);
        ...
        public KeyValuePair<TPriority, TValue> Dequeue()
        {
            if (!IsEmpty)
            {
                ...
                DeleteRoot();
                Console.WriteLine("Из очереди извлекается {0} c приоритетом {1}",  result.Value,result.Key);
                //result
                return result;
            }
            else
        ...
        {
            //Console.WriteLine("Из очереди извлекается {0}", PeekValue());
